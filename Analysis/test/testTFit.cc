...
void testTFit(TString fname){
  ...
  TString stroutputcore=strindir+"/fitterTests/Tfit/";
  ...
  BOOK_BRANCH(float, bestmatchtftop_mass, tout);
  BOOK_BRANCH(std::vector<float>, Tfit_jetpt, tout);
  BOOK_BRANCH(std::vector<float>, Tfit_jeteta, tout);
  BOOK_BRANCH(std::vector<float>, Tfit_jetphi, tout);
  BOOK_BRANCH(std::vector<float>, Tfit_jetmass, tout);
  BOOK_BRANCH(std::vector<float>, Tfit_jetrelresolution, tout);
  BOOK_BRANCH(std::vector<float>, Tfit_jetnuisance, tout);
  BOOK_BRANCH(int, Tfit_status, tout);
  BOOK_BRANCH(int, Tfit_VId, tout);
  BOOK_BRANCH(float, mW_prefit, tout);
  BOOK_BRANCH(float, mW_postfit, tout);
  BOOK_BRANCH(float, mtop_prefit, tout);
  BOOK_BRANCH(float, mtop_postfit, tout);
  BOOK_BRANCH(float, besttftop_fittedTop_dR, tout);
  BOOK_BRANCH(float, bestmatchtftop_fittedTop_dR, tout);
  ...
  plotvars.emplace_back("mW_prefit", "m_{qq} (pre-fit) (GeV)", 60, 70, 100);
  plotvars.emplace_back("mW_postfit", "m_{qq} (post-fit) (GeV)", 60, 70, 100);
  ...
  for (int ev=0; ev<nEntries; ev++){
    ...
    bestmatchtftop_mass = -1;
    bestmatchtftop_fittedTop_dR = -1;
    besttftop_mass = -1;
    besttftop_fittedTop_dR = -1;
    float besttftop_score = -1;
    ...
    for (size_t ip=0; ip<ntftops; ip++){
      ...
      if (besttftop_score<0.f || tftops_disc->at(ip)>besttftop_score){
        besttftop_score = tftops_disc->at(ip);
        besttftop_mass = v.M();
    ...
    Tfit_jetphi.clear();
    Tfit_jetmass.clear();
    Tfit_jetrelresolution.clear();
    Tfit_jetnuisance.clear();
    mW_prefit = -1;
    mW_postfit = -1;
    mtop_prefit = -1;
    mtop_postfit = -1;
    ...
    if (bestpdfval>0.f){
      Tfit_VId=6;
      mW_prefit = sqrt(bestWmass_prefit)/RooRelBW3ProngPdf::GeVunit;
      mW_postfit = sqrt(bestWmass)/RooRelBW3ProngPdf::GeVunit;
      mtop_prefit = sqrt(bestTmass_prefit)/RooRelBW3ProngPdf::GeVunit;
      mtop_postfit = sqrt(bestTmass)/RooRelBW3ProngPdf::GeVunit;
      Tfit_jetpt = topfit_jetpt;
      Tfit_jeteta = topfit_jeteta;
      Tfit_jetphi = topfit_jetphi;
      Tfit_jetmass = topfit_jetmass;
      Tfit_jetrelresolution = topfit_jetrelresolution;
      Tfit_jetnuisance = topfit_jetnuisance;
      Tfit_status = bestfitstatus;
      //MELAout << "\t- Best masses for W and top: " << mW_prefit << ", " << mtop_prefit << " -> " << mW_postfit << ", " << mtop_postfit << endl;
      if (Tfit_jetpt.size()>=3){
        TLorentzVector p_top_prefit;
        for (unsigned int i=0; i<3; i++){ TLorentzVector pjet; pjet.SetPtEtaPhiM(Tfit_jetpt.at(i), Tfit_jeteta.at(i), Tfit_jetphi.at(i), Tfit_jetmass.at(i)); p_top_prefit += pjet; }

        if (bestTFTop) besttftop_fittedTop_dR = bestTFTop->deltaR(p_top_prefit);

        for (size_t ip=0; ip<ntftops; ip++){
          auto const& tftop = tftops.at(ip);
          float tmpdr = tftop.deltaR(p_top_prefit);
          if (bestmatchtftop_fittedTop_dR<0.f || tmpdr<bestmatchtftop_fittedTop_dR){
            bestmatchtftop_fittedTop_dR = tmpdr;
            bestmatchtftop_mass = tftop.m();
          }
        }
      }
    }
    ...
    for (size_t ivar=0; ivar<nvars; ivar++){
      ...
      else if (var.name.BeginsWith("mW_postfit")){ if (mW_postfit<0.f){ continue; } var.setVal(mW_postfit, genweights); }
      else if (var.name.BeginsWith("mW_prefit")){ if (mW_prefit<0.f){ continue; } var.setVal(mW_prefit, genweights); }
